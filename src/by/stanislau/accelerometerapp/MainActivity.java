...
public class MainActivity extends Activity implements OnClickListener {
	...
	void writeAngleXlSFile() {
		// create file in directory
		File file = new File(sdPath, fileNameAngle);
		...
		try {
			...
			try {
				...
				int y1 = 1;
				for (int i = 0; i < globalI; i++) {
					double angle = Math.round(Math.toDegrees(Math
							.atan(Solution(arrayAlpha_y.get(i),
									arrayAlpha_x.get(i)))));
					sheet.addCell(new Label(1, y1, Double.toString(angle)
							.replace('.', ',')));
					y1++;
				}
				int y2 = 1;
				for (int i = 0; i < globalI; i++) {
					double angle = Math.round(Math.toDegrees(Math
							.atan(Solution(arrayAlpha_x.get(i),
									arrayAlpha_y.get(i)))));
					sheet.addCell(new Label(2, y2, Double.toString(angle)
							.replace('.', ',')));
					y2++;
				}
				int y3 = 1;
				for (int i = 0; i < globalI; i++) {
					double angle = Math.round(Math.toDegrees(Math
							.atan(Solution(arrayAlpha_x.get(i),
									arrayAlpha_z.get(i)))));
					sheet.addCell(new Label(3, y3, Double.toString(angle)
							.replace('.', ',')));
					y3++;
				}
	...
	void writeProectionXlSFile() {
		...
		try {
			...
			Label labe20 = new Label(2, 0, "alpha_y");
			Label labe30 = new Label(3, 0, "alpha_z");
			Label labe40 = new Label(4, 0, "v_x");
			Label labe50 = new Label(5, 0, "v_y");
			Label labe60 = new Label(6, 0, "v_z");
			Label labe70 = new Label(7, 0, "s_x");
			Label labe80 = new Label(8, 1, "s_y");
			Label labe90 = new Label(9, 0, "s_z");
			try {
				sheet.addCell(labe00);
				sheet.addCell(labe10);
				sheet.addCell(labe20);
